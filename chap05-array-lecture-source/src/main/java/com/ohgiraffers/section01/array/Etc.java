...
public class Etc {
    ...
    //Poohbao po = new Poohbao (); Monster 와 Poohbao 는 자료형이 다른 것입니다.

    //배열의 한계 : 다른 자료형을 넣을 수 없어요.
    //객체배열이란 클래스를 여러개 저장한다는 뜻인데요 
    //클래스는 String[] names 라고 배열을 만들면 이름과 관련된 String 타입 여러개를 넣을 수 있어요 그러면 나이는 수집못하잖아요
    //클래스는 이름 나이 성별 키가 있다고 하면 자료형이 다 다른데 한꺼번에 묶을 수 없어요 변수를 다다다 많들어야해요
    //규칙이 없어서 반복할수도 없어요. 배열이 있으면 인덱스 규칙이 있는데 변수가 따로 따로 있으면 출력하거나 할때 각자 따로 넣어주어야해요
    //그래서 변수의 한계를 극복하고자 클래스가 나와요
    //지금까지 변수를 메소드 내부에다가 작성했어요 인제는 메인메소드 내부가 아닌 클래스 내부에 작성할거에요
