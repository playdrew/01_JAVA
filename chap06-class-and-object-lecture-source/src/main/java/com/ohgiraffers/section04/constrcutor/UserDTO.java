package com.ohgiraffers.section04.constrcutor;
...
public class UserDTO {
    ...
    private String pwd;
    private String name;
    private LocalDateTime enrollDate; // 필드로 다른 클래스 자료형도 사용가능(회원가입일시)
    ...
    *   3.작성한 생성자 외에는 인스턴스를 생성자는 방법을 제공하지 않겠다
    ...
    *  */
    ...
    /*Index. 1. 기본 생성자*/
    //매개변수가 없으면 기본생성자라고 합니다.
    //기본 생성자는 자동 생성됩니다. 그러나 컴파일에러가 뜨는데 UserDTO user = new UserDTO();
    //매개변수가 있는 생성자를 지우면 매개변수에러 그니까 매개변수가 있는 생성자를 사용하려면 기본생성자를 호출해야합니다.
    public UserDTO(){
        System.out.println("UserDTO 의 기본생성자 호출됨... ");
    }

    // 동일한 이름을 가진 생성자 혹은 메소드는 한 클래스 안에서 작성하는 건 불가능
    //public UserDTO(){} 예외도 있음

    /*Index. 2. 매개변수가 있는 생성자*/
    public UserDTO(String id, String pwd , String name){
        /*comment.
        *   id,pwd,name 초기화를 당담하는 생성자
        *   - 분명 동일한 이름을 사용했지만, 컴파일 오류가 발생하고 있지 않다
        *   - 매개변수 선언부에 작성한 매개변수의 타입 , 갯수 순서에 따라
        *   - 동일한 이름을 가진 생성자 혹은 매소드를 여러 개 작성 할 수 있다는 뜻이다.
        *   - 오버로딩(overloading)*/
        this.id = id;
        this.pwd = pwd;
        this.name = name;
